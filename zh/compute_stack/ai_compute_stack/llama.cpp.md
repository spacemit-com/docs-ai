...
本小节以K3为例，讲解多推理并行的使用和限制，同时，引进一种监控推理资源的工具，spacemit-tcm-smi。

### 3.1. spacemit-tcm-smi

spacemit-tcm-smi是实时监控并清理推理资源的工具，使用方法如下：
...
这时候无法再同时运行新的推理，需要等到该推理完成，tcm状态全部变为free后（如下），才可以运行其他推理。
